...
public class FuturesB {
    ...
    public static void run() throws Exception {
        ...
        try {
            ...
            Future<String> f1 = executor.submit(new Service.CallToRemoteServiceA());
            Future<String> f3 = executor.submit(new Service.CallToRemoteServiceC(f1.get()));
            ...
            Future<Integer> f2 = executor.submit(new Service.CallToRemoteServiceB());
            Future<Integer> f4 = executor.submit(new Service.CallToRemoteServiceD(f2.get()));
            Future<Integer> f5 = executor.submit(new Service.CallToRemoteServiceE(f2.get()));
    ...
    public static void run3() throws Exception {
        ...
        try {
            // kick of f1/f2 in parallel
            final Future<String> f1 = executor.submit(new Service.CallToRemoteServiceA());
            Future<Integer> f2 = executor.submit(new Service.CallToRemoteServiceB());

            // spawn in another thread so waiting on f1 for f3 doesn't block f4/f5
            Future<String> f3 = executor.submit(() -> {
                // get f3 with dependent result from f1 (blocks)
                return new Service.CallToRemoteServiceC(f1.get()).call();
            });
    ...
    public static void run4() throws Exception {
        ...
        try {
            ...
            futures.add(executor.submit(new Service.CallToRemoteServiceA()));
            futures.add(executor.submit(new Service.CallToRemoteServiceB()));
            futures.add(executor.submit(new Service.CallToRemoteServiceC("A")));
            futures.add(executor.submit(new Service.CallToRemoteServiceC("B")));
            futures.add(executor.submit(new Service.CallToRemoteServiceC("C")));
            futures.add(executor.submit(new Service.CallToRemoteServiceD(1)));
            futures.add(executor.submit(new Service.CallToRemoteServiceE(2)));
            futures.add(executor.submit(new Service.CallToRemoteServiceE(3)));
            futures.add(executor.submit(new Service.CallToRemoteServiceE(4)));
            futures.add(executor.submit(new Service.CallToRemoteServiceE(5)));
